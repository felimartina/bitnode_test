...
var ticks_dal = {
    ...
    add_stat: function (tick_id, stat, callback) {
        db.collection('ticks').update(tick_id, { $push: { stats: stat } }, function (err) {
            if (err) {
    ...
    read_last_ticks: function (timestamp, ticks_to_read, skip, callback) {
        ...
        db.collection('ticks').find(filter, filter_options, function (err, docs) {
            if (err) {
    ...
    get_last_valid_stat: function (stat_id, callback) {
        ...
        db.collection('ticks').aggregation(filter, {}, function (err, docs) {
            if (err) {
